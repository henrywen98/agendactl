...
- **v2.1（本次，对外开源）**：改名 **`macctl` → `agendactl`**（GitHub 撞名 + SEO，ADR-0012）；形态收为**单个 `agendactl` skill 内置预编译 universal2 二进制**（装即用，反转 v2.0 的两-skill 拆分，ADR-0012）；定位为 **agent skill**、**不做 MCP server**（Apple-MCP 竞品饱和、skill 才是 agent 的分发渠道，重申 ADR-0008/ADR-0011）。目录 `bin/`、`lib/`、`mcp/` 删除，`.pi/skills/` → `skills/agendactl/`。
...
| 调用模式 | **CLI-only**：agent 只调 `agendactl` 命令、传参，**永不写代码**；无逃生舱 |
| 入口 | **`skills/agendactl/scripts/agendactl`**（Swift；内置预编译 universal2 ad-hoc 签名二进制，旁附 `.swift` 源码 + `build.sh`，ADR-0012） |
| 命令 | calendar / reminders 各 CRUD + 容器列举；固定粗筛 flag，复合查询由 agent 在上下文内筛 |
| 是否用 MCP | **不做**（Apple-MCP 竞品饱和；skill 即 agent 分发渠道，ADR-0011） |
| Skill | 单个 `skills/agendactl/` 覆盖 calendar + reminders，内置二进制,CLI-only 工作流（ADR-0012） |

一句话：**agent 调 `agendactl reminders create --list 任务 --name ...`（只传参）；CLI 内部用 EventKit 处理授权、时区、稳定 id、校验；CLI 没覆盖的就告诉用户「做不了」或由维护者加命令。能力封顶在 CLI 覆盖面，但出错面与维护成本都降一个量级。**
...
- **G5** 可扩展：加 App = 加 `agendactl <app>` 子命令 + 一个 skill 目录（若该 App 无框架则单独引底座）。
...
- ❌ 不做持久化——状态都在原生 App 里，agendactl 是读写器不是存储层。
...
| 命名 / 打包 | 二进制 `agendactl`；calendar / reminders 拆两个独立 skill | [0010] |
...
```
支持 skill 的 agent（Claude Code / Codex / Cursor / Gemini CLI / pi / 脚本 / cron）
  └─ 读 skills/agendactl/SKILL.md（触发即载），只调：agendactl <app> <cmd> --flag …（只传参，永不写码）
        │ bash（跑内置二进制）
        ▼
┌──────────────────────────────────────────────────────────┐
│  skills/agendactl/scripts/agendactl  —— Swift，CLI-only 薄壳        │
│  ├─ 参数解析 + 校验（缺必填 / end>start / 容器名 …）        │
│  ├─ EventKit 调用（授权、查询、CRUD）                       │
│  └─ 输出裸 JSON（stdout）/ 错误 `agendactl: …`（stderr）       │
└───────────────┬──────────────────────────────────────────┘
...
```
agendactl/                          # 项目根（GitHub repo 名 = agendactl）
├── README.md  / README.zh-CN.md  # 人读总览（英文主 + 中文副）
├── ARCHITECTURE.md              # 本文档（设计契约）
├── LICENSE                      # MIT
├── skills/
│   └── agendactl/                   # 自包含、装即用的 skill（覆盖 calendar + reminders）
│       ├── SKILL.md             # 单个 skill（description 路由防误触发）
│       └── scripts/
│           ├── agendactl            # 内置预编译 universal2 + ad-hoc 签名二进制（运行时）
│           ├── agendactl.swift      # 源码（SSOT，可审计 / 可重建）
│           └── build.sh         # 2×swiftc → lipo → codesign
├── tests/
│   ├── smoke.sh                 # 契约检查（help/dispatch/退出码，无授权、无副作用）
...
> 注：v1.0 的 `lib/std.js`（JXA helper）与 `bin/`、`lib/` 目录已删——EventKit 逻辑全在 `skills/agendactl/scripts/agendactl` 内。

---

## 4. 统一契约

所有 `agendactl` 命令遵守以下契约，agent 据此稳定解析。
...
| `1` | 用法/参数错（缺必填、未知命令、`end<=start`） | 看 stderr 改参数；查 `agendactl <app> --help` |
...
- **失败**：退出码非 0，stderr 首行 `agendactl: <message>`，可选 `hint:` / `available:` 行。
...
agendactl --help / agendactl calendar --help / agendactl reminders --help
```

**示例**：
```bash
# 「明天 9 点提醒我交报告」
agendactl reminders create --list "任务" --name "交报告" --due "2026-06-26T09:00:00"
# → {"id":"...","name":"交报告","list":"任务","due":"2026-06-26T01:00:00.000Z"}

# 「把那个提醒改到后天」
agendactl reminders update <id> --due "2026-06-28T09:00:00"

# 「这周有什么会」
agendactl calendar list-events --from "2026-06-22T00:00:00" --to "2026-06-29T00:00:00"
```
...
- **扩命令 ≠ 改运行时**：加子命令只动 `skills/agendactl/scripts/agendactl` + 对应 SKILL.md。
...
└── scripts/
    ├── agendactl           # 内置预编译 universal2 + ad-hoc 签名二进制
    ├── agendactl.swift     # 源码（SSOT）
    └── build.sh        # 可复现构建
```
...
- 交代二进制位置：`agendactl` 在 PATH，否则用**随 skill 内置**的 `scripts/agendactl`（防 agent 满盘 `find`）。

> 不做 MCP server（ADR-0011）：Apple-MCP 赛道已饱和，且 skill 本身就是给 agent 的分发方式；跑不了 shell 的纯 GUI 客户端有意不覆盖。
...
| P1 运行时 | `skills/agendactl/scripts/agendactl`（calendar + reminders，EventKit） | ✅ 实测全绿 |
...
- **CLI-only** — agent 只调 `agendactl` 命令、不写代码；能力封顶 = CLI 覆盖面（ADR-0002）。
- **薄壳** — `agendactl <app> <cmd>`，参数化稳函数 + 校验，不做查询引擎（ADR-0001）。
