...
name: ekctl
description: >-
  Read and write macOS Calendar events AND Reminders/to-dos through the bundled `ekctl` CLI
  ...
  CALENDAR = events with a start/end interval (meetings; NO "complete" concept) → `ekctl calendar …`;
  REMINDERS = to-dos with a due date that can be completed → `ekctl reminders …`.
...
# macOS Calendar & Reminders automation (ekctl)

Drive macOS Calendar and Reminders by calling the `ekctl` CLI. **Only call commands and pass
...
The command is `ekctl`. If the shell reports `command not found` (not on `PATH`), the binary is
**bundled next to this skill at `scripts/ekctl`** — call it by that path; subcommands and flags are
identical. Do not `find` the whole disk for it, and do not substitute another tool (Google Calendar,
a separate task app): this machine's Calendar/Reminders data lives only in macOS, and only `ekctl`
reads it.

## Design notes (read first)

- **Responses carry meta.** Every `ekctl` response has `_at` / `_tz` / `_iso` / `_note` at the top
...
- **Get "today" from the response, not from context.** When the user says "today / tonight /
  tomorrow", read the date from any `ekctl` response's `._iso` field (e.g. `2026-06-29`). Do not
...
- **CLI-only.** Only call `ekctl <app> …` with arguments. Never write code, never touch
...
- **Command map** (same verbs, different prefix — don't cross them):
  ...
  |---|---|---|
  | list containers | `ekctl calendar calendars` | `ekctl reminders lists` |
  | list items | `ekctl calendar list-events` | `ekctl reminders list` |
  | create | `ekctl calendar create-event` | `ekctl reminders create` |
  | update | `ekctl calendar update-event <id>` | `ekctl reminders update <id>` |
  | complete | (none — events have no "complete"; use `delete-event`) | `ekctl reminders complete <id>` |
  | delete | `ekctl calendar delete-event <id>` | `ekctl reminders delete <id>` |

## Calendar commands

| command | purpose |
|---|---|
| `ekctl calendar calendars` | list all calendars: name + id + writable. **Run before any write.** |
| `ekctl calendar list-events [--calendar <name>] [--from <iso>] [--to <iso>] [--limit <n>]` | list events (default next 30 days), returns a JSON array |
| `ekctl calendar create-event --calendar <name> --summary <title> --start <iso> --end <iso> [--location <text>] [--notes <text>] [--all-day]` | create an event, returns one with id |
| `ekctl calendar update-event <id> [--summary] [--start] [--end] [--location] [--notes]` | update (fields not given stay unchanged) |
| `ekctl calendar delete-event <id>` | delete an event |
...
| command | purpose |
|---|---|
| `ekctl reminders lists` | list all lists: name + id + writable. **Run before any write.** |
| `ekctl reminders list [--list <name>] [--status incomplete\|completed\|all] [--due today\|<iso>] [--limit <n>]` | list reminders (default incomplete), returns a JSON array |
| `ekctl reminders create --list <name> --name <text> [--notes <text>] [--due <iso>] [--priority 0-9]` | create a reminder, returns one with id |
| `ekctl reminders update <id> [--name] [--notes] [--due] [--priority] [--complete]` | update (fields not given stay unchanged) |
| `ekctl reminders complete <id>` | mark complete |
| `ekctl reminders delete <id>` | delete |
...
On failure, stderr's first line is `ekctl: <reason>`.
