...
cp -R agendactl/skills/agendactl ~/.claude/skills/agendactl     # Claude Code;或 .pi/skills、~/.codex/skills 等
```
skill 内置了二进制,装上即可用。(`git clone` 不打 quarantine 标记,二进制直接跑;若你从浏览器下的 ZIP,清一次:`xattr -dr com.apple.quarantine ~/.claude/skills/agendactl/scripts/agendactl`。)
...
- 成功 → 退出码 `0`,stdout 为**裸 JSON**(无信封);失败 → 退出码非 0,stderr 首行 `agendactl: <原因>`。
